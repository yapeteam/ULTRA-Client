...
public final class HudUtil {
    ...
    public static void renderQuad(Tessellator tessellator, int x, int y, int width, int height, int color) {
        Color c = ColorUtils.getColorFromHex(color);
        tessellator.getWorldRenderer().begin(7, DefaultVertexFormats.POSITION);
        tessellator.getWorldRenderer().color(c.getRed(),c.getGreen(),c.getBlue(),c.getAlpha());
        tessellator.getWorldRenderer().pos((x), (y), 0.0D);
        tessellator.getWorldRenderer().pos((x), (y + height), 0.0D);
        tessellator.getWorldRenderer().pos((x + width), (y + height), 0.0D);
        tessellator.getWorldRenderer().pos((x + width), (y), 0.0D);
        tessellator.draw();
    }
