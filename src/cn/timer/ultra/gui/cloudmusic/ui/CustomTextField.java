...
public class CustomTextField {
    ...
    public void draw(float x, float y) {
        ...
        //RenderUtil.startGLScissor(this.x + 1, this.y + 3, 113, 11, false, 1f);
        FontLoaders.joystick18.drawString(this.textString, this.x + 1.5f - this.offset, this.y + 4F, Color.GRAY.getRGB());

        if (this.isFocused) {
            float width = FontLoaders.joystick18.getStringWidth(this.textString.substring(0, selectedChar)) + 4;
            float posX = this.x + width - this.offset;
            RenderUtil.drawRect(posX - 0.5f, this.y + 5.5f, posX, this.y + 12.5f, ColorUtils.reAlpha(Color.GRAY.getRGB(), ticks / 500 % 2 == 0 ? 1f : 0));
        }
    ...
    public void mouseClicked(int mouseX, int mouseY, int mouseID) {
        boolean hovering = RenderUtil.isHovering(this.x, this.y + 3f, this.x + 115, this.y + 15, mouseX, mouseY);
        ...
        if (this.isFocused) {
            ...
            switch (key) {
                ...
                case Keyboard.KEY_INSERT:
                    ...
                    this.selectedChar = this.textString.length();
                    width = FontLoaders.joystick18.getStringWidth(this.textString.substring(0, this.selectedChar))
                            + 2;
                ...
                case Keyboard.KEY_BACK:
                    try {
                        ...
                        if (this.textString.length() != 0) {
                            ...
                            --this.selectedChar;
                            if (FontLoaders.joystick18.getStringWidth(oldString) + 2 > 111F && this.offset > 0.0F) {
                                newTextWidth = FontLoaders.joystick18.getStringWidth(this.textString) + 2;
                                oldTextWidth = FontLoaders.joystick18.getStringWidth(oldString) + 2;
    ...
    public void charTyped(char c) {
        if (this.isFocused && ChatAllowedCharacters.isAllowedCharacter(c)) {
            ...
            if (this.selectedChar > this.textString.length()) {
                ...
                ++this.selectedChar;
                float width = FontLoaders.joystick18.getStringWidth(this.textString.substring(0, this.selectedChar)) + 2;
