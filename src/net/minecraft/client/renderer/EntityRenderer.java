...
public class EntityRenderer implements IResourceManagerReloadListener {
    ...
    private float getFOVModifier(float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            if (useFOVSetting) {
                targetf = this.mc.gameSettings.fovSetting;

                if (Config.isDynamicFov()) {
                    targetf *= this.fovModifierHandPrev + (this.fovModifierHand - this.fovModifierHandPrev) * partialTicks;
                }
            }
            ...
            if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).getHealth() <= 0.0F) {
                ...
                targetf /= (1.0F - 500.0F / (f1 + 500.0F)) * 2.0F + 1.0F;
            }

            Block block = ActiveRenderInfo.getBlockAtEntityViewpoint(this.mc.theWorld, entity, partialTicks);

            if (block.getMaterial() == Material.water) {
                targetf = targetf * 60.0F / 70.0F;
            }
            currentf = getAnimationStateSmooth(targetf, currentf, 0.07f / (Minecraft.getDebugFPS() / 60f));//Zoom Animation (created by TIMER_err)
            if (Float.isNaN(currentf))
                currentf = (int) targetf;
            return Reflector.ForgeHooksClient_getFOVModifier.exists() ? Reflector.callFloat(Reflector.ForgeHooksClient_getFOVModifier, this, entity, block, partialTicks, (int) currentf) : (int) currentf;
        }
    }

    private static float getAnimationStateSmooth(float target, float current, float speed) {
    ...
    /*private void orientCamera(float partialTicks) {
        Entity entity = this.cn.timer.ultra.lunarspoof.getMc.getRenderViewEntity();
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (!this.cn.timer.ultra.lunarspoof.getMc.gameSettings.debugCamEnable) {
                BlockPos blockpos = new BlockPos(entity);
                IBlockState iblockstate = this.cn.timer.ultra.lunarspoof.getMc.theWorld.getBlockState(blockpos);
                Block block = iblockstate.getBlock();

                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera, this.cn.timer.ultra.lunarspoof.getMc.theWorld, blockpos, iblockstate, entity);
        ...
        } else if (this.cn.timer.ultra.lunarspoof.getMc.gameSettings.thirdPersonView > 0) {
            ...
            if (this.cn.timer.ultra.lunarspoof.getMc.gameSettings.debugCamEnable) {
                ...
                if (this.cn.timer.ultra.lunarspoof.getMc.gameSettings.thirdPersonView == 2) {
                    f2 += 180.0F;
                }
                ...
                for (int i = 0; i < 8; ++i) {
                    ...
                    f4 = f4 * 0.1F;
                    f5 = f5 * 0.1F;
                    MovingObjectPosition movingobjectposition = this.cn.timer.ultra.lunarspoof.getMc.theWorld.rayTraceBlocks(new Vec3(d0 + (double) f3, d1 + (double) f4, d2 + (double) f5), new Vec3(d0 - d4 + (double) f3 + (double) f5, d1 - d6 + (double) f4, d2 - d5 + (double) f5));
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.cn.timer.ultra.lunarspoof.getMc.gameSettings.debugCamEnable) {
                ...
                Block block1 = ActiveRenderInfo.getBlockAtEntityViewpoint(this.cn.timer.ultra.lunarspoof.getMc.theWorld, entity, partialTicks);
        ...
        } else if (!this.cn.timer.ultra.lunarspoof.getMc.gameSettings.debugCamEnable) {
            GlStateManager.rotate(entity.prevRotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks, 1.0F, 0.0F, 0.0F);
        ...
        this.cloudFog = this.cn.timer.ultra.lunarspoof.getMc.renderGlobal.hasCloudFog(d0, d1, d2, partialTicks);
    }*/
    private void orientCamera(float partialTicks) {
        Entity entity = this.mc.getRenderViewEntity();
        float f = entity.getEyeHeight();
        ...
        if (entity instanceof EntityLivingBase && ((EntityLivingBase) entity).isPlayerSleeping()) {
            ...
            if (!this.mc.gameSettings.debugCamEnable) {
                ...
                if (Reflector.ForgeHooksClient_orientBedCamera.exists()) {
                    Reflector.callVoid(Reflector.ForgeHooksClient_orientBedCamera,
                            this.mc.theWorld, blockpos, iblockstate, entity);
                } else if (block == Blocks.bed) {
                ...
                GlStateManager.rotate(
                        FreeLook.getCameraPrevYaw()
                                + (FreeLook.getCameraYaw() - FreeLook.getCameraPrevYaw()) * partialTicks + 180.0F,
                        0.0F, -1.0F, 0.0F);
                GlStateManager.rotate(
                        FreeLook.getCameraPrevPitch()
                                + (FreeLook.getCameraPitch() - FreeLook.getCameraPrevPitch()) * partialTicks,
                        -1.0F, 0.0F, 0.0F);
            }
            ...
            if (this.mc.gameSettings.debugCamEnable) {
                ...
                float f1 = FreeLook.getCameraYaw();
                float f2 = FreeLook.getCameraPitch();
                ...
                GlStateManager.rotate(FreeLook.getCameraPitch() - f2, 1.0F, 0.0F, 0.0F);
                GlStateManager.rotate(FreeLook.getCameraYaw() - f1, 0.0F, 1.0F, 0.0F);
                GlStateManager.translate(0.0F, 0.0F, (float) (-d3));
                GlStateManager.rotate(f1 - FreeLook.getCameraYaw(), 0.0F, 1.0F, 0.0F);
                GlStateManager.rotate(f2 - FreeLook.getCameraPitch(), 1.0F, 0.0F, 0.0F);
            }
        } else {
            GlStateManager.translate(0.0F, 0.0F, -0.1F);
        }
        ...
        if (Reflector.EntityViewRenderEvent_CameraSetup_Constructor.exists()) {
            if (!this.mc.gameSettings.debugCamEnable) {
                float f6 = FreeLook.getCameraPrevYaw()
                        + (FreeLook.getCameraYaw() - FreeLook.getCameraPrevYaw()) * partialTicks + 180.0F;
                float f7 = FreeLook.getCameraPrevPitch()
                        + (FreeLook.getCameraPitch() - FreeLook.getCameraPrevPitch()) * partialTicks;
                float f8 = 0.0F;
                ...
                Object object = Reflector.newInstance(Reflector.EntityViewRenderEvent_CameraSetup_Constructor,
                        this, entity, block1, partialTicks, f6,
                        f7, f8);
                Reflector.postForgeBusEvent(object);
                f8 = Reflector.getFieldValueFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_roll, f8);
                f7 = Reflector.getFieldValueFloat(object, Reflector.EntityViewRenderEvent_CameraSetup_pitch, f7);
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime) {
        ...
        if (FreeLook.overrideMouse() && flag) {
            ...
            if (this.mc.currentScreen != null) {
                ...
                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        ...
                        EventManager.instance.call(new EventRenderScreen(k1, l1, this.mc.currentScreen, partialTicks));
