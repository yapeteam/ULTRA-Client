...
import cn.timer.ultra.module.modules.render.Rotations;
...
public abstract class RendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
    ...
    public void doRender(T entity, double x, double y, double z, float entityYaw, float partialTicks) {
        boolean isPlayer = Minecraft.getMinecraft().thePlayer.equals(entity);
        boolean enable = Client.instance.rotationUtils.rotating && Client.instance.getModuleManager().getByClass(Rotations.class).isEnabled();
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entity, this, x, y, z)) {
            ...
            try {
                ...
                float f1;
                if (enable && isPlayer) {
                    f = this.interpolateRotation(Client.instance.rotationUtils.prevRotation.getYaw(), Client.instance.rotationUtils.rotation.getYaw(), partialTicks);
                    f1 = this.interpolateRotation(Client.instance.rotationUtils.prevRotation.getYaw(), Client.instance.rotationUtils.rotation.getYaw(), partialTicks);
                } else {
                ...
                float f8 = 0;
                if (enable && isPlayer) {
                    f8 = Client.instance.rotationUtils.prevRotation.getPitch() + (Client.instance.rotationUtils.rotation.getPitch() - Client.instance.rotationUtils.prevRotation.getPitch()) * partialTicks;
                } else {
        ...
        if (!enable && isPlayer) {
            Client.instance.rotationUtils.prevRotation.setRotation(entity.rotationYaw, entity.rotationPitch);
        }
    }
