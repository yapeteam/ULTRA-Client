...
import static pisi.unitedmeows.meowlib.encryption.hashing.HashingHelper.*;
...
public enum MeowHash {
    ...
    public byte[] getHash(final byte[] message) {
        ...
        while (tOutputLen > 0) {
            ...
            for (int i = 0; i < blockSize; i++) byteResults.write((byte) uState[i]);
            tOutputLen -= blockSize;
            if (tOutputLen > 0) doKeccakf(uState);
        }
    ...
    private void doKeccakf(final int[] uState) {
        ...
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 5; j++) {
                final int[] data = convertFrom64ToLittleEndian(lState[i][j]);
    ...
    private void roundInteger(final BigInteger[][] state) {
        ...
        for (int round = 0; round < 24; round++) {
            ...
            for (int i = 0; i < 24; i++) {
                ...
                x = y;
                y = (2 * tX + 3 * y) % 5;
                final BigInteger shiftValue = current;
                current = state[x][y];
            ...
            for (int j = 0; j < 5; j++) {
                ...
                for (int i = 0; i < 5; i++) {
                    final BigInteger invertVal = t[(i + 1) % 5].xor(BIG_PRIME);
                    state[i][j] = t[i].xor(invertVal.and(t[(i + 2) % 5]));
                }
            }
            for (int i = 0; i < 7; i++) {
                LFSRstate = ((LFSRstate << 1) ^ ((LFSRstate >> 7) * 0x71)) % 256;
                final int bitPosition = (1 << i) - 1;
                if ((LFSRstate & 2) != 0) state[0][0] = state[0][0].xor(new BigInteger("1").shiftLeft(bitPosition));
            }
        }
    }
}
